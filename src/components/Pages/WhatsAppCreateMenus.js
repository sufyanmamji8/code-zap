...
const WhatsAppCreateMenus = () => {
  ...
  const createSubMenu = (parentMenu) => {
    // Use the exact parent menuId and menuTitle instead of modifying them
    const parentMenuId = parentMenu.menuId;
    const parentMenuTitle = parentMenu.menuTitle;
    
    setFormData({
      ...createInitialMenuState(),
      menuId: parentMenuId,      // Use the exact same ID as parent
      menuTitle: parentMenuTitle // Use the exact same title as parent
    });
    ...
    <>
      ...
      <Container fluid className="d-flex flex-column flex-md-row p-0">
        ...
        <div 
          className={`
            menu-sidebar 
            ${sidebarCollapsed ? 'menu-sidebar-collapsed' : ''}
            ${isMobile ? 'mobile-sidebar' : ''}
          `}
          style={{
            width: isMobile 
              ? (sidebarCollapsed ? '0' : '80%') 
              : (sidebarCollapsed ? '80px' : '250px'),
            minHeight: '93vh',
            background: '#fff',
            ...
            left: isMobile && sidebarCollapsed ? '-100%' : 0
          }}
        >
        ...
        <div className="flex-grow-1 position-relative">
          <div className="whatsapp-menu-container">
            ...
            <div className="menu-header-new d-flex flex-wrap justify-content-between align-items-center">
              <div className="d-flex align-items-center">
                ...
                <div>
                  <h2 className="mb-1 responsive-title">WhatsApp Menu Builder</h2>
                  <p className="text-muted d-none d-md-block">
                    Build and manage your interactive WhatsApp menus
                  </p>
                </div>
              </div>
              ...
              <Button 
                color="primary" 
                className="add-menu-btn-new mt-2 mt-md-0"
                onClick={toggleModal}
              >
                <Plus size={20} className="me-2" /> 
                <span className="d-none d-md-inline">Create New Menu</span>
                <span className="d-md-none">New Menu</span>
              </Button>
            </div>
  ...
  <Input
    ...
    // Remove the 'disabled={isFirstMenu}' that was preventing editing of main menu ID
    required
    placeholder="Enter a unique menu ID"
  />
  <small className="text-muted">
    Enter a unique identifier for this menu. Use lowercase letters, numbers, and hyphens.
  </small>
</FormGroup>
                    </Col>
                    <Col md={6}>
