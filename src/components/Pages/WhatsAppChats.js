  ...
  try {
    ...
    if (response.data.success) {
      ...
      const formattedContacts = contacts.map(contact => ({
        phoneNumber: contact.phoneNumber === response.data.businessNumber ? contact._id : contact.phoneNumber,
        senderName: contact.name || contact.phoneNumber,
        lastMessage: "",
        timestamp: Date.now().toString(),
        flag: countryList.find(c => contact.phoneNumber.startsWith(c.code))?.flag || '🌐'
      }));

      setContacts(prev => {
        const merged = [...prev];
        formattedContacts.forEach(newContact => {
          if (!merged.some(c => c.phoneNumber === newContact.phoneNumber)) {
