...
const WhatsAppChats = () => {
  ...
  const getStatusIcon = (status, messageId) => {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      if (status === 'failed') {
        toast.error(`Message failed: ${failureReason || "Unknown error"}`);
        setMessages(prev => prev.map(msg =>
          msg.messageId === tempMessageId ? { ...msg, status: 'failed', failureReason } : msg
        ));
      }
