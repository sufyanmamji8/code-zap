...
useEffect(() => {
  if (businessId) {
    ...
    socketRef.current.on(`onmessagerecv-${businessId}`, async () => {
      ...
      // First fetch new messages if we're in a chat
      if (selectedUser) {
        await fetchMessages(selectedUser.phoneNumber);
      }
      
      // Then fetch latest contacts
      const response = await axios.post(
      ...
      if (response.data.success) {
        ...
        // Update contacts state with new order
        setContacts(prevContacts => {
          // Combine new contacts with existing ones not in the first page
...
}, [businessId, selectedUser, companyId, token]);
  ...
  const renderNewChatModal = () => (
    <Modal isOpen={isNewChatModal} toggle={() => setIsNewChatModal(false)}>
      <ModalHeader toggle={() => setIsNewChatModal(false)}>
        New Chat
      </ModalHeader>
      <ModalBody>
        <div style={{ display: "flex", gap: "10px", marginBottom: "15px" }}>
                ...
                >
                  ...
                  <div style={{ fontSize: "11px", color: "#888" }}>
                    {latestData.timestamp
                      ? new Date(parseInt(latestData.timestamp) * 1000).toLocaleTimeString('en-US', {
                          hour: 'numeric',
                          minute: '2-digit',
                          hour12: true
                        })
