...
// Reusable Component for WhatsApp Account Card
const WhatsAppAccountCard = ({ account }) => (
  <Col lg="4" md="6" sm="12" className="mb-4">
    ...
    >
      ...
      <Button
        ...
        onClick={() => toast.success("Open WhatsApp clicked!")}
      >
        Open WhatsApp
      </Button>
    </Card>
...
const Dashboard = () => {
  ...
  const fetchWhatsappAccounts = async () => {
    ...
    try {
      ...
      if (res.data.success && Array.isArray(res.data.data)) {
        ...
        throw new Error("Failed to fetch accounts or no accounts available.");
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (location.state?.newAccountAdded) {
    ...
    <>
      ...
      {loading ? (
        ...
        <Row className="mx-auto">
          {whatsappAccounts.length > 0 ? (
            whatsappAccounts.map((account) => (
              <WhatsAppAccountCard key={account._id} account={account} />
            ))
          ) : (
            <div style={{ textAlign: "center", width: "100%", color: "#6c757d" }}>
              No WhatsApp accounts found.
